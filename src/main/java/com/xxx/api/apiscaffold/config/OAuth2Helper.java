...
public class OAuth2Helper {
    ...
    private final Logger log = LoggerFactory.getLogger(OAuth2Helper.class);
    ...
    public static String getAccessToken(HttpServletRequest request) {
        // 优先获取 cookie 中的 access_token
        Cookie accessCookie = getAccessTokenCookie(request);
        if (accessCookie != null && org.apache.commons.lang3.StringUtils.isNotBlank(accessCookie.getValue())) {
            LOGGER.info("get access token from cookie success.");
            return accessCookie.getValue();
        }
        // 获取请求头中的 Authorization 中的 access_token
        String authorization = request.getHeader(AUTHORIZATION);
        if (org.apache.commons.lang3.StringUtils.isNotBlank(authorization)) {
            LOGGER.info("get access token from request head success.");
            return authorization.substring(OAuth2AccessToken.BEARER_TYPE.length()).trim();
        }

        LOGGER.info("ready to get access token from parameter.");
        // 返回参数中的 access_token
        return request.getParameter(OAuth2AccessToken.ACCESS_TOKEN);
    }

    public static String getRefreshToken(HttpServletRequest request) {
        String refreshToken;
        // 优先获取 cookie 中的 refresh_token
        Cookie refreshCookie = getRefreshTokenCookie(request);
        if (refreshCookie != null && org.apache.commons.lang3.StringUtils.isNotBlank(refreshCookie.getValue())) {
            LOGGER.info("get refresh token from cookie success.");
            return refreshCookie.getValue();
        }
        // 返回请求投中的 refresh_token
        refreshToken = request.getHeader(REFRESH_TOKEN_COOKIE);
        if (org.apache.commons.lang3.StringUtils.isNotBlank(refreshToken)) {
            LOGGER.info("get refresh token from request head success.");
            return refreshToken;
        }
        // 返回 session_token (表示是会话级别的token)
        refreshToken = request.getHeader(SESSION_TOKEN_COOKIE);
        if (org.apache.commons.lang3.StringUtils.isNotBlank(refreshToken)) {
            LOGGER.info("get session token from request head success.");
            return refreshToken;
        }
        LOGGER.info("ready to get refresh token from parameter.");
        // 返回参数中的 refresh_token
    ...
    public static boolean isRememberMe(HttpServletRequest request) {
        ...
        return StringUtils.hasText(request.getHeader(REFRESH_TOKEN_COOKIE));
    }
     ...
     */
    public boolean isSessionExpired(String refreshToken) {
        //read non-remember-me session length in secs
        int validity = customConfig.getSecurity().getSessionTimeoutInSeconds();
        if (validity < 0) {           //no session expiration configured
            return false;
        }
        Integer iat = getClaim(refreshToken, "iat", Integer.class);
    ...
    private String getCookieDomain(HttpServletRequest request) {
        String domain = customConfig.getSecurity().getCookieDomain();
        if (domain != null) {
            return domain;
        }
